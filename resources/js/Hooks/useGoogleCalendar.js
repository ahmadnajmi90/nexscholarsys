...
export function useGoogleCalendar() {
    ...
    const checkStatus = useCallback(async () => {
        try {
            setIsLoading(true);
            const response = await axios.get(route('supervision.google-calendar.status'));
            ...
            logError(error, 'useGoogleCalendar checkStatus');
    ...
    /**
     * Get Google OAuth URL and open in popup
     */
    const connect = useCallback(async () => {
        try {
            ...
            logError(error, 'useGoogleCalendar connect');
    ...
    const disconnect = useCallback(async () => {
        try {
            ...
            logError(error, 'useGoogleCalendar disconnect');
            toast.error('Failed to disconnect Google Calendar');
        }
    }, []);

    /**
     * Add a specific meeting to Google Calendar
     */
    const addMeetingToCalendar = useCallback(async (meetingId) => {
        try {
            const response = await axios.post(
                route('supervision.google-calendar.add-meeting', { meeting: meetingId })
            );
            ...
            if (error.response?.status === 403) {
                toast.error('You do not have permission to add this meeting');
            } else if (error.response?.data?.already_exists) {
                toast('This meeting is already in your Google Calendar', {
                    icon: 'ℹ️',
                    duration: 3000,
                });
                return { success: false, alreadyExists: true, meeting: error.response.data.meeting };
            } else if (error.response?.data?.needs_connection) {
                toast.error('Please connect Google Calendar first');
            } else {
                logError(error, 'useGoogleCalendar addMeetingToCalendar');
                toast.error('Failed to add meeting to Google Calendar');
            }
            return { success: false };
        }
    }, []);
    ...
    const syncFromGoogle = useCallback(async () => {
        try {
            const response = await axios.post(route('supervision.google-calendar.sync'));
            
            if (response.data.success) {
                const { synced, updated, deleted } = response.data;
                if (synced > 0) {
                    toast.success(`Synced ${synced} meeting(s) from Google Calendar`);
                } else {
            ...
            logError(error, 'useGoogleCalendar syncFromGoogle');
