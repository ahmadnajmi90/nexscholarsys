...
const SearchBar = ({ 
  ...
  // Handle clicks outside search bar to close it (mobile only)
  useEffect(() => {
  ...
  // Handle search bar toggle (mobile only)
  ...
  // Desktop view: Always show full search bar
  if (!isMobile) {
    return (
      <div className={`search-bar-container relative ${className}`}>
        <div className="flex items-center bg-white border border-gray-300 rounded-lg shadow-lg overflow-hidden">
          ...
          <input
            type="text"
            className="w-80 bg-transparent text-gray-900 text-sm p-2.5 focus:outline-none focus:ring-0 border-0"
  ...
  // Mobile view: Expandable search bar (retain current behavior)
  return (
    <div className={`search-bar-container relative ${className}`}>
