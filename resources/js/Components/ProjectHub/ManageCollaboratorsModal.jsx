...
export default function ManageCollaboratorsModal({ show, onClose, context, contextType, connections }) {
    ...
    const contextId = context?.id || (contextType === 'workspace' && context?.data?.id);
    
    // Determine if the current user is the owner
    const isOwner = auth.user.id === (context?.owner_id || (contextType === 'workspace' && context?.data?.owner_id));
    
    // Get the members array based on context type
    const members = contextType === 'workspace' ? context?.data?.members : context?.members;
                    ...
                    <div className="mb-8">
                        ...
                        {members && members.length > 0 ? (
                            <div className="space-y-3">
                                {members
                                    .filter(member => member.id !== (context?.owner_id || (contextType === 'workspace' && context?.data?.owner_id)))
                                    .map(member => {
