...
import ProgramCard from '@/Components/Postgraduate/ProgramCard';
...
export default function Results({ recommendations = [], selectedProgram = null, supervisors = [] }) {
  ...
  const pageTitle = !selectedProgram ? 'Postgraduate Program Recommendations' : 'Available Supervisors';
  
  const handleViewSupervisors = (program) => {
    if (!program?.id) return toast.error('Program not found');
    router.get(route('postgraduate-recommendations.supervisors', program.id));
  };
  ...
  const filtered = useMemo(() => {
    ...
    if (query) {
      ...
      list = list.filter(rec => {
        const program = rec.postgraduate_program;
          ...
          <>
            ...
            <div className="flex items-start gap-3 bg-green-50 border border-green-200 text-green-900 px-4 py-3 rounded-md mb-5">
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col ml-1 text-sm">
                  ...
                  <p>We found {filtered.length} highly compatible Postgraduate Programs based on your profile and research interests.</p>
                </div>
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-5">
              {filtered.map((rec, idx) => (
                <ProgramCard key={idx} rec={rec} onViewSupervisors={() => handleViewSupervisors(rec.postgraduate_program)} />
              ))}
            </div>
          ...
          <div>
            ...
            {!isLoadingSupervisors && supervisors.length > 0 && (
              <div className="text-center mt-12 py-8 border-t border-gray-200">
                ...
                <Link
                  href={route('postgraduate-recommendations.results')}
                  className="inline-flex items-center px-6 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50"
                >
                  <ArrowLeft className="w-4 h-4 mr-2" />
                  Back to All Programs
