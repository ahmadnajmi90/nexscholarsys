...
const GrantCard = ({ grants }) => {
  ...
  const uniqueGrantTypes = [...new Set(grants.map((grant) => grant.grant_type))].filter(Boolean);
  const uniqueGrantThemes = [
    ...new Set(grants.flatMap((grant) => grant.grant_theme || [])),
  ].filter(Boolean);
  const uniqueCountries = [...new Set(grants.map((grant) => grant.country))].filter(Boolean);
  ...
  const filteredGrants = grants.filter((grant) => {
    ...
    const matchesTheme =
      grantThemeFilter.length === 0 || grant.grant_theme?.some((theme) => grantThemeFilter.includes(theme));
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {displayedGrants.map((grant, index) => (
            ...
            >
              ...
              />
              <div className="p-6">
                <h2 className="text-xl font-semibold text-gray-800 truncate" title={grant.title}>
                  {grant.title}
                </h2>
                <p
        ...
        >
          ...
          >
            ...
            <p className="text-gray-600 mb-2">
                ...
                {Array.isArray(selectedGrant.grant_theme) ? selectedGrant.grant_theme.join(", ") : "No Grant Theme Specified"}
            </p>
