...
export default function PhDProgramFormModal({ isOpen, onClose, mode = 'create', currentProgram = null, onSuccess }) {
  ...
  useEffect(() => {
    ...
    if (isEdit) {
      ...
      setForm({ name: '', university_id: '', faculty_id: '', description: '', duration_years: '', funding_info: '', application_url: '', country: '' });
    }
  ...
  const submitManual = async (e) => {
    ...
    try {
      if (isEdit) {
        await axios.post(`/api/v1/phd-programs/${currentProgram.id}`, form);
        toast.success('Program updated');
      } else {
        await axios.post('/api/v1/phd-programs', form);
  ...
  const handlePreviewImport = async (e) => {
    ...
    try {
      const response = await axios.post('/api/v1/phd-programs/import/preview', formData, { headers: { 'Content-Type': 'multipart/form-data' } });
  ...
  const handleConfirmImport = async () => {
    setIsImportLoading(true);
    try {
      await axios.post('/api/v1/phd-programs/import', { programs: previewData });
            ...
            >
              <DialogPanel className="w-full max-w-2xl transform overflow-hidden rounded-lg bg-white p-0 text-left align-middle shadow-xl transition-all">
                <div className="px-6 py-4 border-b flex items-center justify-between">
                  <h3 className="text-lg font-medium">{isEdit ? 'Edit PhD Program' : 'Add PhD Program'}</h3>
              ...
              {importStep === 'upload' && (
                <form onSubmit={handlePreviewImport} className="space-y-4">
                  <div className="bg-blue-50 border border-blue-200 p-3 rounded text-sm text-blue-900">
                    ...
                    <p>Upload a .xlsx or .csv file with a header row containing these columns: <strong>name, university_id, faculty_id, description, duration_years, funding_info, application_url, country</strong>.</p>
                    <a href="/storage/templates/phd_programs_template.csv" className="text-blue-700 underline" target="_blank" rel="noreferrer">Download template</a>
                  </div>
                  <div>
