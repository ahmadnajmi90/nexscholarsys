   ...
   - [Connections & Social Features](#connections--social-features)
   ...
   **Important**: Nexscholar uses Laravel 11 which has several key architectural changes from previous versions:
   ...
   ### Centralized Connections System

   Nexscholar uses a centralized `connections` table to manage various types of user relationships. This system offers flexibility and efficiency by handling different relationship types in a single structure:

   - **Connection Types**: The system supports multiple relationship types through the `type` column:
     - `bookmark`: One-way relationship for saving users for future reference
     - `friendship`: Two-way connection between users requiring mutual agreement
     - `supervision`: Specialized academic relationship (e.g., supervisor-student)

   - **Connection Status**: Each connection has a status that reflects its current state:
     - `pending`: A request has been sent but not yet accepted
     - `accepted`: The connection has been established and is active
     - `rejected`: The connection request was declined
     - `blocked`: The connection is actively prevented

   ### Friend Request System

   The platform implements a comprehensive friend request workflow:

   1. **Sending Requests**: Users can send connection requests to other users via the Connect button on profiles
   2. **Request Management**: Users can view, accept, or reject incoming requests in their Connections page
   3. **Connection Removal**: Either party can remove an established connection at any time

   ### Bookmarking System

   The bookmark feature allows users to save profiles they're interested in without requiring mutual acceptance:

   - Bookmarks are implemented as a special type of connection with automatic `accepted` status
   - Users can add and remove bookmarks from profile pages
   - Bookmarked profiles are accessible from the Connections page

   ### API Endpoints

   The following API endpoints are available for managing connections:

   - `GET /connections`: Display the connections management page
   - `GET /connections/get`: Get connections of specific type and status
   - `POST /connections/bookmark/toggle`: Toggle bookmark status for a user
   - `GET /connections/bookmark/check`: Check if a user is bookmarked
   - `POST /connections/friend/request`: Send a friend request to a user
   - `POST /connections/{connection}/accept`: Accept a pending friend request
   - `POST /connections/{connection}/reject`: Reject a pending friend request
   - `DELETE /connections/remove`: Remove an established connection
   - `GET /connections/status`: Check connection status between users

   > **Note**: All connection endpoints are protected by authentication middleware to ensure data privacy and security.
