...
class GeneratePhDProgramEmbeddings extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'embeddings:generate-phd-programs
     ...
     * @var string
     */
    protected $description = 'Generate and store embeddings for all PhD programs';
    ...
    public function handle()
    {
        ...
        $collection = 'nexscholar_phd_programs';
        $this->qdrantService->createCollection($collection);

        $total = PhDProgram::count();
        if ($total === 0) {
            $this->info('No PhD programs found.');
            return Command::SUCCESS;
        }

        $this->info("Generating embeddings for {$total} PhD programs (batch size: {$batchSize})");
        ...
        PhDProgram::with(['university', 'faculty'])
            ->orderBy('id')
            ->chunk($batchSize, function ($programs) use (&$processed, &$failed, $progressBar, $collection) {
                foreach ($programs as $program) {
                    try {
                        ...
                        $failed++;
                        Log::error('Error embedding PhD program ID ' . $program->id . ': ' . $e->getMessage());
                    } finally {
                        $progressBar->advance();
                    }
                }
    ...
    private function constructProgramText(PhDProgram $program): string
    {
        $parts = [];
